...
void Trainer::train_joint()
{
    ...
    for (long i = 0; i < params.init_GN_iter; i++) {
        ...
        std::vector<std::vector<cv::Mat> > rhs_samplef1 = mtimesx(init_samplef_proj, yf, 1);

        // Construct the right hand side vector for the projection matrix part
        std::vector<std::vector<cv::Mat> > fyf = mtimesx(hf, yf, 1);
        ...
        const std::vector<std::vector<cv::Mat> >  &init_hf, float proj_reg)
{
    ...
    std::vector<std::vector<cv::Mat> >       fAndDel = hf.up_part;    // f + delta(f)
        ...
        std::vector<std::vector<cv::Mat> > &hf)                   // the union of filter [f+delta(f) delta(p)]
{
    ...
    std::vector<std::vector<cv::Mat> > x = hf;      // initialization of CG
...
float Trainer::inner_product_joint(const joint_out &a, const joint_out &b)
{
    ...
    for (long i = 0; i < a.up_part.size(); i++) {
        for (long j = 0; j < a.up_part[i].size(); j++) {
            ...
            ip = ip + 2 * mat_sum(temp1) - mat_sum(temp2);
        }
        cv::Mat temp = real(complexMultiplication(mat_conj(a.low_part[i].clone()), b.low_part[i]));
        ip += mat_sum(temp);
    }
    return ip;
}
...
Trainer::rl_out   Trainer::rl_out::operator+(rl_out data2)
{
    rl_out res;
...
Trainer::rl_out   Trainer::rl_out::operator*(float scale)
{
    rl_out res;
    res.up_part = Features::FeatScale(up_part, scale);
    res.low_part = Features::ProjScale(low_part, scale);
    return res;
}

Trainer::rl_out   Trainer::rl_out::operator-(rl_out data)
{
    rl_out res;
